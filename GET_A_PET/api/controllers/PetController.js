...
module.exports = class PetController {
    ...
    static async removePetById(req,res){
        ...
        if(pet.user._id.toString() !== user._id.toString()){
            res.status(422).json({message: 'O pet não pertence ao usuário logado!'})
            return
        }

        try {
            const deletedPet = await Pet.findByIdAndDelete(id)
            res.status(200).json({message: 'Pet removido com sucesso!',deletedPet})
        } catch(error) {
            res.status(500).json({message: error})
        } 
    ...
    static async schedule(req,res) {
        ...
        if(pet.user._id.equals(user._id)){
            res.status(422).json({message: 'Você não pode agendar visita para seu próprio pet!'})
            return
        }
    ...
    static async concludeAdoption(req,res){
        ...
        if(!pet.user._id.equals(user._id)){
            res.status(422).json({message: 'Você não pode finalizar a adoção de um pet que não é seu!'})
            return
        }
        ...
        try {
            ...
            res.status(200).json({message: 'Parabéns! O ciclo de adoção foi finalizado com sucesso.',pet})
        } catch(error) {
            res.status(500).json({message: error})
        }
